...
//	  System.out.println("Inputs: " + result);
		...
		if (result.size() < 2) {
			...
			Bpmain.bpclassifyNo(result.get(0),result.get(1),result.get(2));
			System.out.println(bp1 + bp6 + bp2 + bp5 +"\n");
			System.out.println(" BT [ " + result.get(3) + "°C ] (measured Forehead temperature)");
			Filewriter_oneline.main(bp1 + bp6 + bp2 + bp5+"\n");
			Filewriter_oneline.main("BT [ " + result.get(3) + "°C ] (measured Forehead temperature)");
		}
		 else if (result.size() < 6) {
			Bpmain.bpclassifyNo(result.get(0),result.get(1),result.get(2));
			System.out.println(bp1 + bp6 + bp2 + bp5 +"\n");
			System.out.println("BT [ " + result.get(3) + "°C ] (measured Forehead temperature)");
			System.out.println("RR [ " + result.get(4) + " ] breaths /min (measured while sitting)");
			Filewriter_oneline.main(bp1 + bp6 + bp2 + bp5+"\n");
			Filewriter_oneline.main("BT [ " + result.get(3) + "°C ] (measured Forehead temperature)");
			Filewriter_oneline.main("RR [ " + result.get(4) + " ] breaths /min (measured while sitting)");
		 }
		 else {
		}
  }
